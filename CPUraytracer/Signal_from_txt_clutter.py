# -*- coding: utf-8 -*-
"""
Created on Tue Dec 10 12:17:57 2024

@author: thoms
"""

'''
calculating signal from clutter
'''
...
from signal_calc_functions import func_chirp, find_azi_number_fkt, find_start_end_fkt
...
def signal_from_distance_weighted_fkt(distance_vec, ref_vec): #, center_frequency, Bandwidth, chirp_duration):
    f_center = 10e9 # Hz
    Bandwidth = 4e9 # Hz
    ...
    for n in range(0, len(time_collected_rays)):
        displacement_time = time_collected_rays[n]
        spec_index = int((displacement_time/delta_t))
        total_spec[spec_index : int(spec_index + puls_time_steps)] += spec_puls_vec * ref_vec[n]
    print(time.time()-t0)
...
def convolution_spec_calc(distance_vec, ref_vec):
    delta_d = 0.001
    max_d = 360
    #chirp signal
    T=5*10**-8
    ...
    range_vec = np.zeros(indgange, np.complex_)
    for i in range(int(len(distance_vec))):
        div = float(distance_vec[i]) / delta_d
        index = np.floor(div)
        range_vec[int(index)] += 1 * ref_vec[i]
...
def SAR_signal_fkt(name, center_frequency, Bandwidth, chirp_duration):
    ...
    names = fnmatch.filter(os.listdir('C:\\Users\\thoms\\.spyder-py3\\Optimering af ray tracer\\'), 'clutter_azi_*' + name + '*.txt')
    azi_steps = len(names)

    azi_start, azi_end = find_start_end_fkt(names[0],name)
    azimuthal_vec =  np.linspace(azi_start, azi_end, azi_steps)

    data = np.loadtxt("C:\\Users\\thoms\\.spyder-py3\\Optimering af ray tracer\\" + names[0], delimiter=',', dtype = np.complex128)
    distance_vec = data[:,0]
    weight_vec = data[:,1]
    ...
    for i in range(azi_steps):
        ...
        data = np.loadtxt("C:\\Users\\thoms\\.spyder-py3\\Optimering af ray tracer\\" + names[i], delimiter=',', dtype = np.complex128)
        distance_vec = data[:,0]
        weight_vec = data[:,1]
        # signal_out = convolution_spec_calc(distance_vec, weight_vec)
        signal_out = signal_from_distance_weighted_fkt(distance_vec, weight_vec)
    ...
    np.save("clutter_" + name + "_Matrix", DATA_matrix)
    np.save("clutter_" + name + "_range_vec", range_vec)
    np.save("clutter_" + name + "_azimuth_vec", azimuthal_vec)
    
    return DATA_matrix, range_vec, azimuthal_vec
    
SAR_signal_fkt("tree_rand_reim", 10e9, 4e9, 5e-8)
